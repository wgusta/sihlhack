...
;
;
function RunPanel({ challengeId, scenarioId, onScenarioChange, onRun, onCancel, currentRunStatus, canRun, isSubmitting, devModeAllowed, devModeEnabled, onDevModeEnabledChange }) {
...
function buildSceneFrame(challengeId, run, artifacts) {
    ...
    setNode('compute', {
        ...
        status: criticalCount > 0 ? 'critical' : warningCount > 0 ? 'warning' : 'active'
    });
    setNode('battery', {
    ...
    setNode('house', {
        value: round(Math.max(0.6, computeW / 2600), 2),
        unit: 'kW',
    ...
    setNode('sensor', {
        value: events.length,
        unit: 'events',
        pulse: clamp01(events.length / 12),
        status: statusFromSeverity(warningCount, criticalCount)
    });
    const safetyState = String(metrics.final_state || metrics.mode || 'NORMAL').toUpperCase();
    setNode('safety', {
        value: safetyState.includes('LOCKOUT') || safetyState.includes('E_STOP') ? 3 : safetyState.includes('CRITICAL') ? 2 : 1,
        unit: safetyState,
    ...
    const mode = challengeId === 'grid-os' ? String(metrics.mode || 'AUTO') : challengeId === 'safety-coordination' ? String(metrics.final_state || 'NORMAL') : 'SENSOR';
    return {
        challengeId,
        mode,
        nodes,
...
//# sourceMappingURL=_22b23a78._.js.map